...
import com.kui.community.util.MailClient;
...
//    private LoginTicketMapper loginTicketMapper;
    ...
    public Map<String, Object> register(User user) {
        ...
        // 注册用户
        user.setSalt(CommunityUtil.generateUUID().substring(0, 5)); // 生成加密salt
        user.setPassword(CommunityUtil.md5(user.getPassword() + user.getSalt())); //设置加密后的密码
        user.setType(0); // 0为普通用户
        user.setStatus(0); // 0为未激活状态
    ...
    public Map<String, Object> login(String username, String password, long expiredSeconds) {
        ...
        // 验证状态
        if(user.getStatus() == 0) {
            map.put("usernameMsg", "该账号未激活，请前往邮箱激活！");
            return map;
        }
        // 验证密码
